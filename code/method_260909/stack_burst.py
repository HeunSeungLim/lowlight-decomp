...
for k in keys:
    ...
    ym=np.mean([np.load(f"{CACHE}/{i:04d}.npy").transpose(1,2,0).astype(np.float32) for i in idx],0)
    MQ.append(np.percentile(ym,QL)); MA.append(float((ym*g).sum()/((ym*ym).sum()+1e-12)))
    MB.append(p8(g8(ym),G)); MS.append(k[0]); MY.append(k)
    MSINGLE.append(float(np.mean([p8(g8(np.load(f"{CACHE}/{i:04d}.npy").transpose(1,2,0)),G) for i in idx])))
...
for te in folds:
    ...
    sub=np.where(tr)[0]
    def cs(v): return float(np.mean([p8(g8(np.mean([np.load(f"{CACHE}/{i:04d}.npy").transpose(1,2,0) for i in grp[MY[j]]],0)*v[j]),g8(gt_of(MS[j])))-MB[j] for j in sub[::2]]))
    lam=LAMS[int(np.argmax([0.0 if l==0 else cs(1+l*(p-1)) for l in LAMS]))]
    for j in np.where(~tr)[0]:
        ym=np.mean([np.load(f"{CACHE}/{i:04d}.npy").transpose(1,2,0).astype(np.float32) for i in grp[MY[j]]],0)
