...
M = {
    ...
    "RTWOin": f"{E['r2']['cin']:.3f}", "RTWOall": f"{E['r2']['call']:.3f}",
...
_lf = json.load(open("numbers/diag_sid_lowfreq.json"))["overall"]
...
for tag in ("woperc", "wperc", "retinexformer"):
    av = json.load(open(f"numbers/measure_LOLv1_{tag}.json"))["avg"]
...
_cv = json.load(open("numbers/measure_SID.json"))
...
_gs = json.load(open("numbers/newbase_gsad.json"))
def _find(d, keys):
    for k in keys:
...
_ur = json.load(open("numbers/newbase_uretinex.json"))
_urmd = open("numbers/NEWBASE_uretinex.md").read()
...
_ldj = json.load(open("numbers/newbase_lightendiff.json"))["seed_sweep_psnr_round"]; _lo, _hi = _ldj["min"], _ldj["max"]
...
_bx = E.get("by_exposure") or json.load(open("numbers/diag_sid_failure.json")).get("by_exposure")
